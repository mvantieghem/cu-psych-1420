...
Let's see if age correlates with Year in school. The function `cor.test()` allows us to put in two variables, see what their correlation is, and return a p-value for that correlation.
...
First, we'll save the contents of our correlation test into the variable `cor_age_school` so we can access its contents later. Then, we'll print `cor_age_school` to the console, so we can visually inspect the results of the correlation test.

```{r}
cor_age_school <- cor.test(IntroSurvey$Age, IntroSurvey$School, method = "pearson")
...
Let's see if the two maximizing scales are correlated (MTS_total and MS_total) Use `cor.test()` on those two variables, and report the Pearson's r and p-value.
...
Typically, we'd expect these two scales to be very correlated. Does this appear to be the case in our dataset? 
